...
impl ConfigLoader {
    ...
    pub fn save_config(
        ...
        self.change_log.push(ConfigChangeLog::New(
            config_name.clone(),
            from_lang.clone(),
            target.clone(),
        ));
    ...
    pub fn delete_config(&mut self, config_name: &ConfigName) -> anyhow::Result<()> {
        self.change_log
            .push(ConfigChangeLog::Delete(config_name.clone()));
        Ok(())
    }
}
impl AsyncClose for ConfigLoader {
    async fn async_close(self) -> anyhow::Result<()> {
        ...
        let to_delete = self
            .change_log
            .iter()
            .filter(|x| x.is_delete())
            .map(|x| x.as_delete())
            .collect::<Vec<ConfigName>>();
        ...
        File::options()
            ...
            .await?;
        let file = File::options().write(true).open(&self.config_path).await?;
        ...
        for line in last_file_buf.split(|x| *x == b'\n') {
            if let Ok(Some(cfg)) = TransConfig::from_one_line(std::str::from_utf8(line)?) {
                ...
                buf_writer.write(&cfg.to_line()).await?;
            }
        }
        for log in &self.change_log {
            if let ConfigChangeLog::New(config_name, from, target) = log {
                buf_writer
                    .write(&TransConfig::from_val_to_line(config_name, from, target))
                    .await?;
            }
        }
